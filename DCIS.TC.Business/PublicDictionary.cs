   ...
   public class PublicDictionary
    {
       ...
       /// <param name="data"></param>
       /// <returns></returns>
       public dynamic GetViewSaleUser(int start, int limit, dynamic data)
       {
           //UserCode, UserName, PositionName, DepartName, PositionDesc
           string strSql = " SELECT   UserCode as PurUserCode, UserName as PurUserName, PositionName, DepartName, PositionDesc  FROM ViewUser where 1=1 AND DepartName='销售部' ";
           if (data != null)
           {
               if (data.name != null && data.name != "")
                   strSql += string.Format(@" and (UserCode like '%{0}%' OR UserName like '%{0}%' OR DepartName like '%{0}%')", data.name.Value.Trim());
