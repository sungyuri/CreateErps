...
{
  public  class ShipInCfmDataRecord_BLL
    {
      /// <summary>
      /// 进港确报
      /// </summary>
      ...
      public dynamic Get(int start, int limit, string strOrderBy, dynamic data)
      {
          string strSql = @"  SELECT A.*,B.Key_Text as DEPARTURE_PORT_CODE_TEXT,
                            C.KEY_TEXT AS ARRIVAL_PORT_CODE_TEXT,D.KEY_TEXT AS IMPORT_GOODS_TYPE_TEXT
                            FROM TRAN_IN_CONFIRM A
                            ...
                            LEFT JOIN TRAN_KEY D ON A.IMPORT_GOODS_TYPE=D.KEY_VALUE AND D.KEY_TYPE='GOODSTYPE'
      ...
      /// <param name="data"></param>
      /// <returns></returns>
      public dynamic GetCfmShip(int start, int limit, string strOrderBy, dynamic data)
      {
          string strSql = @" SELECT  B.SHIP_NO,B.IMO,B.SHIP_NAME,B.ENTER_NUMBER,B.ABROAD_NUMBER,
                            B.ARRIVAL_DATE,B.BERTH_CODE,B.LOADING_DATE,B.DEPARTURE_PORT_CODE,
                            B.ARRIVAL_PORT_CODE,B.AGENT_CODE,B.CONTEXT_NAME,B.DECLARATION_DATE,B.FREE_TEXT,
                            D.KEY_TEXT AS DEPARTURE_PORT_CODE_TEXT,E.KEY_TEXT AS ARRIVAL_PORT_CODE_TEXT
                            FROM TRAN_IN_FORECAST B 
                            INNER JOIN TRAN_SCHEDULE_INFO C ON  B.SCHEDULE_ID=C.SCHEDULE_ID
                            AND C.SHIP_STATUE='1' AND
                            B.SCHEDULE_ID  NOT IN(SELECT SCHEDULE_ID FROM TRAN_IN_CONFIRM WHERE R_STATUE='0')  
      ...
      /// <returns></returns>
      public string Update(TRAN_IN_CONFIRMEntity entity, string type)
      {
          ...
          try
          {
              ...
              if (type.Equals("1"))
              {
                  ...
                  scientity.SHIP_STATUE = "2";
                  PublicRule.Update(scientity);

                  TRAN_IN_CONFIRM_BAKEntity entityBak = new TRAN_IN_CONFIRM_BAKEntity();
      ...
      /// <param name="entity"></param>
      /// <returns></returns>
      public string Insert(TRAN_IN_CONFIRMEntity entity,string type)
      {
          string returnValue = "";
          try
          {
      ...
      {
          string sql = string.Format(@"delete from TRAN_IN_CONFIRM where SCHEDULE_ID in({0})", strID);
          DBUtil.Fill(sql);
      }
      ...
      {
          string tbNo = "";
          string strSql = " SELECT SCHEDULE_ID FROM  TRAN_SCHEDULE_INFO  WHERE SHIP_NO='" + ship_no + "' AND SHIP_STATUE='1' ";
