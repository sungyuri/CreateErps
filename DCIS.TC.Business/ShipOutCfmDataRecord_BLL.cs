...
namespace TCEPORT.TC.Business
{
    ...
    /// 离港确报
    /// </summary>
  public  class ShipOutCfmDataRecord_BLL
    {
        ...
        public dynamic GetArrShip(int start, int limit, string strOrderBy, dynamic data)
        {
            string strSql = @"SELECT B.SHIP_NO,B.IMO,B.SHIP_NAME,B.ENTER_NUMBER,B.ABROAD_NUMBER,B.ARRIVAL_DATE,
                              B.DEPARTURE_DATE,B.BERTH_CODE,B.START_PORT_CODE ,B.END_PORT_CODE,B.AGENT_CODE,
                              B.CONTEXT_NAME,B.DECLARATION_DATE,B.FREE_TEXT,
                              D.KEY_TEXT AS  START_PORT_CODE_TEXT,E.KEY_TEXT AS END_PORT_CODE_TEXT
                              FROM  TRAN_OUT_FORECAST B 
                              INNER JOIN TRAN_SCHEDULE_INFO C 
                              ON B.SCHEDULE_ID=C.SCHEDULE_ID  AND C.SHIP_STATUE='4' AND
                              B.SCHEDULE_ID  NOT IN(SELECT SCHEDULE_ID FROM TRAN_OUT_CONFIRM WHERE R_STATUE='0') 
                              LEFT JOIN tran_port_code D ON B.START_PORT_CODE=D.KEY_VALUE 
                              LEFT JOIN tran_port_code E ON B.END_PORT_CODE=E.KEY_VALUE 
                              WHERE 1=1   ";
            if (data != null)
            {
                if (data.SHIP_NAME != null && data.SHIP_NAME != "")
        ...
        /// <returns></returns>
        public string Update(TRAN_OUT_CONFIRMEntity entity, string type)
        {
        ...
        /// <param name="entity"></param>
        /// <returns></returns>
        public string Insert(TRAN_OUT_CONFIRMEntity entity,string type)
        {
            string returnValue = "";
            try
            {
        ...
        {
            string sql = string.Format(@"delete from TRAN_OUT_CONFIRM where SCHEDULE_ID in({0})", strID);
            DBUtil.Fill(sql);
        }
        public string queryScNo(string ship_no)
        {
            string tbNo = "";
            string strSql = " SELECT SCHEDULE_ID FROM  TRAN_SCHEDULE_INFO  WHERE SHIP_NO='" + ship_no + "' AND SHIP_STATUE='4' ";
