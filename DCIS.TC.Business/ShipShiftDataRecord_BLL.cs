...
namespace TCEPORT.TC.Business
{
    ...
    /// 移泊报SHIFTING_PORT
    /// </summary>
  public  class ShipShiftDataRecord_BLL
    {
        ...
        /// <returns></returns>
        public dynamic GetShiftShip(int start, int limit, string strOrderBy, dynamic data)
        {
            string strSql = @"SELECT  B.SHIP_NO,B.IMO,B.SHIP_NAME,B.ENTER_NUMBER,B.ABROAD_NUMBER,B.ARRIVAL_DATE,
                              C.BERTH_CODE AS BERTH_NOW_CODE,B.DEPARTURE_PORT_CODE,B.ARRIVAL_PORT_CODE,B.AGENT_CODE,
                              ...
                              B.SCHEDULE_ID  NOT IN(SELECT SCHEDULE_ID FROM TRAN_SHIFTING WHERE R_STATUE='0') 
        ...
        /// <returns></returns>
        public string Update(TRAN_SHIFTINGEntity entity, string type)
        {
            ...
            try
            {
                ...
                if (type.Equals("1"))
                {
                    ...
                    scientity.BERTH_CODE = entity.BERTH_TO_CODE;
                    scientity.SHIP_STATUE = "3";
                    PublicRule.Update(scientity);

                    TRAN_SHIFTING_BAKEntity entityBak = new TRAN_SHIFTING_BAKEntity();
                    entityBak.SHIFTID = entity.SHIFTID;
        ...
        /// <param name="entity"></param>
        /// <returns></returns>
        public string Insert(TRAN_SHIFTINGEntity entity,string type)
        {
            string returnValue = "";
            try
            {
        ...
        {
            string sql = string.Format(@"delete from TRAN_SHIFTING where SHIFTID in({0})", strID);
            DBUtil.Fill(sql);
        }
